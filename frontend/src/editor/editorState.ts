...
interface EditorCommand {
  type: "INCREASE_HEIGHT" | "TOGGLE_FLAG" | "CLEAR_SELECTED_MARKS";
  label: string;
  ids: string[];
  amount?: number;
...
export function setActiveTool(state: EditorState, tool: Tool): EditorState {
  return {
    ...state,
    activeTool: tool,
    messages: [`目前工具：${toolLabels[tool]}`, ...state.messages].slice(0, 6),
  };
}
...
export function handleCellClick(state: EditorState, cellId: string, additive = false): EditorState {
  if (state.activeTool === "select") {
    const selectedIds = additive
      ? toggleValue(state.selectedIds, cellId)
      : [cellId];
    return {
      ...state,
      selectedIds,
  ...
  return applyToolToSelection({ ...state, selectedIds });
}
...
function applyToolToSelection(state: EditorState): EditorState {
...
function undo(state: EditorState): EditorState {
  ...
  if (!historyItem) return { ...state, messages: ["沒有可復原的操作", ...state.messages].slice(0, 6) };
  return {
    ...state,
...
function redo(state: EditorState): EditorState {
  ...
  if (!historyItem) return { ...state, messages: ["沒有可重做的操作", ...state.messages].slice(0, 6) };
  return {
    ...state,
...
function applyCommandToCell(cell: EditorCell, command: EditorCommand): EditorCell {
  ...
  if (command.type === "INCREASE_HEIGHT") return { ...cell, height: cell.height + (command.amount ?? 1) };
  if (command.type === "TOGGLE_FLAG" && command.field) return { ...cell, [command.field]: !cell[command.field] };
